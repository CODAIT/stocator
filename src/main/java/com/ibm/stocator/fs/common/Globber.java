...
public class Globber {
  ...
  public Globber(FileSystem fsT, Path pathPatternT, PathFilter filterT) {
    fs = fsT;
    fc = null;
  ...
  public Globber(FileContext fcT, Path pathPatternT, PathFilter filterT) {
    fs = null;
    fc = fcT;
    pathPattern = pathPatternT;
    filter = filterT;
  ...
  private FileStatus[] doGlob() throws IOException {
    ...
    for (String flatPattern : flattenedPatterns) {
      ...
      for (int componentIdx = 0; componentIdx < components.size();
        ...
        ObjectStoreGlobFilter globFilter = new ObjectStoreGlobFilter(components.get(componentIdx));
