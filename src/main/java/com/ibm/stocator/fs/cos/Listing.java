...
import static com.ibm.stocator.fs.cos.COSUtils.createFileStatus;
import static com.ibm.stocator.fs.cos.COSUtils.objectRepresentsDirectory;
...
public class Listing {
  ...
  public Listing(COSAPIClient ownerT) {
    owner = ownerT;
  }
  ...
  static final class SingleStatusRemoteIterator
     ...
     */
    public SingleStatusRemoteIterator(LocatedFileStatus statusT) {
      status = statusT;
    }
  ...
  class FileStatusListingIterator
     ...
     * @throws IOException IO Problems
     */
    FileStatusListingIterator(ObjectListingIterator sourceT,
        PathFilter filterT,
        FileStatusAcceptor acceptorT,
        RemoteIterator<FileStatus> providedStatusT) throws IOException {
      source = sourceT;
      filter = filterT;
      acceptor = acceptorT;
      providedStatus = new HashSet<>();
      for (; providedStatusT != null && providedStatusT.hasNext();) {
        final FileStatus status = providedStatusT.next();
        if (filter.accept(status.getPath()) && acceptor.accept(status)) {
          providedStatus.add(status);
        }
      }
    ...
    private boolean buildNextStatusBatch(ObjectListing objects) {
      ...
      for (S3ObjectSummary summary : objects.getObjectSummaries()) {
        ...
        if (acceptor.accept(keyPath, summary) && filter.accept(keyPath)) {
          ...
          if (status.getLen() > 0) {
            ...
            LOG.debug("Ignoring Stocator directory? : {}", keyPath);
            ignored++;
          }
        } else {
      ...
      for (String prefix : objects.getCommonPrefixes()) {
        ...
        Path keyPath = new Path(prefix);
        if (acceptor.accept(keyPath, prefix) && filter.accept(keyPath)) {
          FileStatus status = new COSFileStatus(true, false, keyPath);
  ...
  class ObjectListingIterator implements RemoteIterator<ObjectListing> {
    ...
    ObjectListingIterator(
        ...
        ListObjectsRequest request) {
      listPath = listPathT;
      maxKeys = owner.getMaxKeys();
      objects = owner.listObjects(request);
    }
    ...
    public ObjectListing next() throws IOException {
      if (firstListing) {
        ...
        try {
          ...
          throw COSUtils.translateException("listObjects()", listPath, e);
        }
      }
      return objects;
    }
  ...
  static class AcceptFilesOnly implements FileStatusAcceptor {
    ...
    public AcceptFilesOnly(Path qualifiedPathT) {
      qualifiedPath = qualifiedPathT;
    }
  ...
  class LocatedFileStatusIterator
     ...
     * @param statusIterator an iterator over the remote status entries
     */
    LocatedFileStatusIterator(RemoteIterator<FileStatus> statusIteratorT) {
      statusIterator = statusIteratorT;
    }
  ...
  static class TombstoneReconcilingIterator implements
    ...
    TombstoneReconcilingIterator(RemoteIterator<LocatedFileStatus>
        iteratorT, Set<Path> tombstonesT) {
      iterator = iteratorT;
      if (tombstonesT != null) {
        tombstones = tombstonesT;
      } else {
        tombstones = Collections.EMPTY_SET;
      }
    }
  ...
  static class AcceptAllButSelfAndS3nDirs implements FileStatusAcceptor {
     ...
     */
    public AcceptAllButSelfAndS3nDirs(Path qualifiedPathT) {
      qualifiedPath = qualifiedPathT;
    }
