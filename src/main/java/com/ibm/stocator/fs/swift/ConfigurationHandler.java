...
public final class ConfigurationHandler {
  ...
  public static Properties initialize(URI uri, Configuration conf) throws IOException {
    ...
    if (!Utils.validSchema(uri)) {
      ...
      Utils.updateProperty(conf, prefix2D, AUTH_URL, props, SWIFT_AUTH_PROPERTY, true);
      Utils.updateProperty(conf, prefix2D, USERNAME, props, SWIFT_USERNAME_PROPERTY, true);
      Utils.updateProperty(conf, prefix2D, PASSWORD, props, SWIFT_PASSWORD_PROPERTY, true);
      Utils.updateProperty(conf, prefix2D, TENANT, props, SWIFT_TENANT_PROPERTY, true);
      Utils.updateProperty(conf, prefix2D, AUTH_METHOD, props, SWIFT_AUTH_METHOD_PROPERTY, false);
      Utils.updateProperty(conf, prefix2D, BLOCK_SIZE, props, SWIFT_BLOCK_SIZE_PROPERTY, false);
      Utils.updateProperty(conf, prefix2D, FMODE_DELETE_TEMP_DATA, props,
          FMODE_AUTOMATIC_DELETE_PROPERTY, false);
      Utils.updateProperty(conf, prefix2D, PUBLIC, props, SWIFT_PUBLIC_PROPERTY, false);
      ...
      if (authMethod.equals(KEYSTONE_V3_AUTH)) {
        Utils.updateProperty(conf, prefix2D, REGION, props, SWIFT_REGION_PROPERTY, false);
